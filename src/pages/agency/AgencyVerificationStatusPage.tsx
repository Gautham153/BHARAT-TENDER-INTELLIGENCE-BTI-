...
export const AgencyVerificationStatusPage: React.FC<AgencyVerificationStatusPageProps> = ({
  ...
  // Load organization and audit trail
  const loadStatus = async () => {
    setLoading(true);
    try {
      const orgId = user?.organizationId || 'ORG-APEX-27A';
      ...
      if (res.organization) {
        ...
        // Fallback placeholder structure if newly registered
        setOrgData({
          ...
          registeredAddress: 'Registered Commercial Office',
          state: 'Maharashtra',
          ...
          primaryUserId: user?.id || 'usr-temp',
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200">
          <div className="space-y-1">
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600 font-mono">
              <span className="font-bold text-[#002B49]">GSTIN: {orgData?.gstin || user?.gstin}</span>
              <span>•</span>
          ...
          <div className="p-5 bg-amber-50/80 border border-amber-200 rounded-xl space-y-4 animate-fadeIn">
            ...
            <div className="bg-white/90 rounded-lg p-3.5 border border-amber-200 grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
              <div>
                ...
                <span className="font-mono font-bold text-slate-900">{orgData?.applicationId}</span>
              </div>
              <div>
        ...
        {status === 'verified' && (
          <div className="p-5 bg-emerald-50/80 border border-emerald-200 rounded-xl space-y-4 animate-fadeIn">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-emerald-900 leading-relaxed">
                  Your organization has successfully satisfied the statutory verification requirements. Your account is eligible to access verified-agency capabilities including tender discovery and bidding.
                </p>
              </div>
            </div>
            ...
            <div className="p-2.5 bg-emerald-100/60 rounded-lg text-[11px] text-emerald-900 font-medium">
              Note: Verification confirms identity and statutory registration. It is one element of BTI&apos;s overall trust and integrity framework.
            </div>
        ...
        <div className="space-y-3 pt-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-xs">
            <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-1">
              ...
              <span className="font-bold text-slate-900 block">{orgData?.legalName}</span>
            </div>

            <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-1">
              <span className="text-slate-500 block text-[11px]">Registered GSTIN</span>
              <span className="font-mono font-bold text-[#002B49] block">{orgData?.gstin}</span>
            </div>

            <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-1">
              <span className="text-slate-500 block text-[11px]">Registration State / UT</span>
              <span className="font-semibold text-slate-900 block">{orgData?.state}</span>
            </div>

            <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-1">
              <span className="text-slate-500 block text-[11px]">Business Category</span>
              <span className="font-semibold text-slate-900 block">{orgData?.businessCategory}</span>
            </div>
            ...
            <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-1 sm:col-span-2">
              ...
              <span className="text-slate-800 block truncate">{orgData?.registeredAddress}</span>
            </div>
          </div>
        </div>
      </Card>
